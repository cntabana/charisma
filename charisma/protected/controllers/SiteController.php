...
class SiteController extends Controller
{
	...
	public function actionLogin()
	{
		...
		if(isset($_POST['LoginForm']))
		{
			...
			if($model->validate() && $model->login())
				$this->redirect(Yii::app()->user->returnUrl.'?r=site/dashboard');
		}
	...
	public function actionCitydata()
   {
	   ...
	   //load the cities for the current province id (=ECascadeDropDown::submittedKeyValue())
	    $data = Sector::model()->findAll('iddistrict=:iddistrict',array(':iddistrict'=>ECascadeDropDown::submittedKeyValue()));
	   //Convert the data by using 
	   //CHtml::listData, prepare the JSON-Response and Yii::app()->end 
	   ECascadeDropDown::renderListData($data,'id', 'sector');
